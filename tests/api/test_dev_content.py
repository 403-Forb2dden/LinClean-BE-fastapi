"""/api/v1/dev/content/* — 크롤링 결과·콘텐츠 분석 디버그 엔드포인트."""
...
async def client() -> AsyncIterator[httpx.AsyncClient]:
    """dev 라우터만 마운트한 얇은 FastAPI 앱 — lifespan / DB / 스케줄러를 건드리지 않는다."""
    app = FastAPI()
    app.include_router(dev_endpoint.router, prefix=f"{settings.api_v1_prefix}/dev")
    transport = ASGITransport(app=app)
    async with httpx.AsyncClient(transport=transport, base_url="http://test") as c:
        yield c


_ENDPOINT_FETCH = f"{settings.api_v1_prefix}/dev/content/fetch-extract"
_ENDPOINT_ANALYSIS = f"{settings.api_v1_prefix}/dev/content-analysis"
...
class TestFetchExtractEndpoint:
    async def test_returns_fetch_and_features_on_success(self, client: httpx.AsyncClient) -> None:
        ...
        with patch(
            "app.api.v1.endpoints.dev.fetch_page",
...
class TestContentAnalysisEndpoint:
    async def test_returns_full_content_analysis(self, client: httpx.AsyncClient) -> None:
        ...
        with patch(
            "app.api.v1.endpoints.dev.analyze_content",
    ...
    async def test_does_not_skip_based_on_preceding_score(
        ...
        """파이프라인과 달리 dev 엔드포인트는 항상 실제 분석을 수행한다."""
        expected = ContentAnalysisResult(
            final_url="https://x.test/",
            fetched=False,
            score=0,
