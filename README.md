...
`dev/content/*` 엔드포인트는 raw URL 을 받기 때문에 `normalize_url()` 로 한 번 더
...
**원본 대비 ~10× 까지 부푼다** (파서·태그 객체 오버헤드). FastAPI 동시성 N 에서
이 비용이 곱해지므로, 운영 인스턴스 메모리에 맞춰 `CONTENT_FETCH_MAX_BYTES` 를 보수적으로
잡는 것이 안전하다. 2MiB 기본값은 일반 페이지 분석에는 넉넉하나, 메모리 여유가 작은
컨테이너에서는 1MiB 이하로 낮춰 동시 처리 한계를 확보한다.
...
판정을 즉시 읽을 수 있습니다 — 이 구조는 `/api/v1/dev/analyze` 동기 응답과
`run_pipeline()` 결과를 그대로 쓰는 Spring 콜백 페이로드 양쪽에 동일하게
적용됩니다.
...
## 디버그 엔드포인트 (`/api/v1/dev/*`)

`settings.debug=true` 환경에서만 라우터가 마운트되며, 운영 배포에서는 노출되지
않는다. 단계별 단독 호출 + 전체 파이프라인 호출을 모두 동기로 제공해 QA·디버깅에
사용한다. 모든 엔드포인트는 raw URL 을 받아 `normalize_url()` 로 1차 검증한 뒤
해당 단계만 실행한다.
...
| POST   | `/dev/analyze`                  | 전체 (1~4 + verdict)| `run_pipeline`                  |
| POST   | `/dev/content/fetch-extract`    | 4단계 보조 디버그   | `fetch_page` + `extract_features` |

요청 바디는 모두 `{ "url": "<raw url>" }` 형태이며, `/dev/analyze` 는 추가로
`analysisId` 를 내부에서 생성한다. `/dev/threat-db` 는 DB 세션이 필요해
요청 처리 시 `get_db` dependency 가 일반 라우트와 동일하게 주입된다.

**단계별 API 사용 시 주의 — unchainer 는 1단계에서만 동작**: `/dev/threat-db`,
`/dev/domain-heuristic`, `/dev/content-analysis` 는 `normalize_url` 만 거치고
unchain 은 거치지 않는다. 단축 URL(`bit.ly/...`) 을 그대로 넣으면 4단계는
`unexpected_redirect` 로 떨어지고, 2·3단계는 단축 호스트 자체로 매치/조회가
나간다. 단축 URL 을 풀어 분석하려면 먼저 `/dev/normalize` 로 `unchain.final_url` 을
확인한 뒤 그 값을 단계별 API 에 다시 넣는다.

**예시 — Stage 1 (정규화 + 언체이닝)**

```bash
curl -sX POST http://localhost:8000/api/v1/dev/normalize \
...
```bash
curl -sX POST http://localhost:8000/api/v1/dev/content-analysis \
  -H 'Content-Type: application/json' \
  -d '{"url":"https://login-secure-naver-auth.com/signin"}'
```

응답은 `ContentAnalysisResult` 그대로(verdict 포함). `ai_error` 필드는 다음 의미를
갖는다:
...
```json
{
  ...
  "stages": {
    ...
    "contentAnalysis": {
      ...
      "aiModel":   "gpt-4o-mini",
      "aiTokenUsage": { "promptTokens": 412, "completionTokens": 47, "totalTokens": 459 }
    }
  },
